...
const ffipaths: any = {
    linux: app.isPackaged
        ? path.resolve(process.resourcesPath, "linux/lib/libImSDK1.so")
        : path.resolve(
              process.cwd(),
              "node_modules/im_electron_sdk/lib/linux/lib/libImSDK1.so"
          ),
...
function getFFIPath() {
    ...
    switch (platform) {
        case "linux":
            ...
            if (linuxcpu == "arm64") res = ffipaths[linuxcpu];
