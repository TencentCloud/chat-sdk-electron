 ...
 * 2.1 会话事件回调
 *
 * @param conv_event 会话事件类型，请参考[TIMConvEvent]()
 ...
 * 
 * @param cb 会话未读消息总数变更的回调，请参考[TIMConvTotalUnreadMessageCountChangedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 
 * @param cb 按会话 filter 过滤的未读消息总数变更的回调，请参考[TIMConvTotalUnreadMessageCountChangedByFilterCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 5.1 会话分组被创建
 * 
 * @param cb 会话未读消息总数变更的回调，请参考[TIMConvConversationGroupCreatedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 5.2 会话分组被删除
 * 
 * @param cb 会话未读消息总数变更的回调，请参考[TIMConvConversationGroupDeletedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 5.3 会话分组名变更
 * 
 * @param cb 会话未读消息总数变更的回调，请参考[TIMConvConversationGroupNameChangedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 5.4 会话分组新增会话
 * 
 * @param cb 会话未读消息总数变更的回调，请参考[TIMConvConversationsAddedToGroupCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 5.5 会话分组删除会话
 * 
 * @param cb 会话未读消息总数变更的回调，请参考[TIMConvConversationsDeletedFromGroupCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 6.1 获取会话列表
 *
 * @param cb 获取会话列表的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 ...
 * @param cb 会话列表的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 ...
 * @param count 分页拉取的个数
 * @param cb 获取会话列表高级接口的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * @param cb 删除会话成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param cb 删除会话列表的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 ...
 * @param cb 设置会话自定义字段的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * @param cb 设置会话置顶回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 ...
 * @param cb 标记会话的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC，回调 cb 才会被调用）。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 ...
 * 6.11 获取所有会话总的未读消息数
 *
 * @param cb 获取所有会话总的未读消息数回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC，回调 cb 才会被调用）。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param cb 获取未读总数的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param cb 创建会话分组的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 *
 * @param cb 获取会话分组列表的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC，回调 cb 才会被调用）。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 *
 * @param group_name 分组名（必填参数，长度要 > 0，最大支持 32 bytes）
 * @param cb 删除会话分组的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC，回调 cb 才会被调用）。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * @param cb 重命名会话分组的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC，回调 cb 才会被调用）。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * @param group_name 分组名（必填参数，长度要 > 0，最大支持 32 bytes）
 * @param conversation_id_array 会话 ID 列表（必填参数，不能为空）
 * @param cb 添加会话到一个会话分组的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC，回调 cb 才会被调用）。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * @param group_name 分组名（必填参数，长度要 > 0，最大支持 32 bytes）
 * @param conversation_id_array 会话 ID 列表（必填参数，不能为空）
 * @param cb 从会话分组中删除多个会话的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC，回调 cb 才会被调用）。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
...
// uint [TIMGroupAtType](), 只读, @ 提醒类型，分成 “@我” 、“@所有人” 以及 “@我并@所有人” 三类
...
// uint [TIMConvType](TIMCloudComm.h), 只写, 会话类型
...
// uint [TIMConversationMarkType](), 只写, 标记类型
 ...
 * @param conv_id   会话的ID
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param cb 创建会话的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
